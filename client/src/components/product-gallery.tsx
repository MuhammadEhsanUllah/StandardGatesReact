...
export default function ProductGallery() {
  ...
  const filteredProducts = activeCategory === 'All Gates' 
    ? products 
    ...
    <section id="gallery" className="py-16">
      <div className="container mx-auto px-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-12">
          <div>
            <h2 className="font-heading text-3xl font-bold text-slate-900 mb-2">Featured Gate Designs</h2>
            <p className="text-slate-600">Browse our most popular custom gate designs</p>
          </div>
            ...
            <div className="inline-flex rounded-md shadow-sm mt-4 md:mt-0" role="group">
              {categories.map((category, index) => (
                <Button
                  ...
                  className={`${index === 0 ? 'rounded-r-none' : index === categories.length - 1 ? 'rounded-l-none' : 'rounded-none border-l-0'}`}
            ...
            <Card key={product.id} className="overflow-hidden hover:shadow-lg transition-all">
              ...
              <CardContent className="p-4">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-primary font-heading font-bold">${product.price}</span>
